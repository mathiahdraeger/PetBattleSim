...
import UIBased.Model.Calculators.*;
import UIBased.Model.Events.AttackEvent;
import UIBased.Model.Events.AttackEvent.*;
import UIBased.Model.Events.FightStartEvent;
import UIBased.Model.Events.FightStartEvent.*;
import UIBased.Model.Events.PlayerEventInfo;
import UIBased.Model.Events.PlayerEventInfo.*;
import UIBased.Model.Events.RoundStartEvent;
import UIBased.Model.Events.RoundStartEvent.*;
...
public class Referee extends Observable
{
	...
	public void buildFightStartEvent(int fightNumber) 
	{
		...
		playerEventInfoList = buildPlayerEventInfoList();
	 ...
	 * @param index
	 */
	public void chooseSkill(int index, Skills skill)
	{
		Playable currentPlayable = playableList.get(index);
		if(skill != null)
			currentPlayable.setCurrentSkill(skill);
	 ...
	 */
	public String executeAttackEvent()
	{
		if(!attackEventQueue.isEmpty())
		{
			...
			return (playableList.get(attackingIndex).getPetName() + " attacked " 
	 ...
	 * @param index
	 */
	public String printStats(int index)
	{
		if(playableList.get(index).isAwake())
			return (playableStats(index));
		return "";
	}	
	...
	public void setPredictedSkill(int index, Skills skill)
	{
		playableList.get(index).setPredictedSkill(skill);
	}
